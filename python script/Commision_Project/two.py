...
# Harry's Downline - Client-based rates (from reference table)
...
# Confidence special multipliers
...
def extract_date_from_filename(filename):
    ...
    # Try MM_DD_YYYY or MM-DD-YYYY or MMDDYYYY
    patterns = [
...
def process_raw_files():
    ...
    for filepath in valid_files:
        ...
        # Try to get date from file content first, then filename
        check_date = None
        if date_col:
...
def build_full_report(packets):
    ...
    filename = f"Commission_Report_{report_date.strftime('%B_%Y')}.xlsx"
    ...
    # ==============================================================================
    # PHASE 2: PLAN COUNTING SECTION (Below Grand Totals)
    # ==============================================================================
    
    plan_count_start_row = 5  # Position below Grand Totals row
    plan_count_col = totals_col  # Start at Grand Totals column
    ...
    # Determine which plan counting logic to use based on number of weeks
    if num_weeks == 2:
        ...
        # Assuming plan columns are at indices in plan_cols list
        if len(plan_cols) >= 2:
            col_C = xl_col_to_name(plan_cols[0])  # First week Plan column
            col_I = xl_col_to_name(plan_cols[1])  # Second week Plan column
    ...
    # ==============================================================================
    # PHASE 2: HARRY'S DOWNLINE SECTION (Below Plan Counting)
    # ==============================================================================
    
    downline_start_row = plan_count_start_row + 6
    downline_col = totals_col  # Start at same column as Grand Totals
    ...
    # Set column widths for downline section
    ws_comm.set_column(downline_col, downline_col, 25)  # Client/Agent name column
    ws_comm.set_column(downline_col + 1, downline_col + 2, 18)  # Count columns
    ws_comm.set_column(downline_col + 3, downline_col + 3, 15)  # Commission column
    
    current_downline_row += 1
    
    # Process each client in Harry's downline
    for client_name, agents in HARRY_DOWNLINE_RATES.items():
        ...
        for agent_name, rates in agents.items():
            ...
            # Reference to plan count cells (updated to use plan_count_col)
            ...
            # Show counts (just reference the main counts)
            ...
            # Calculate commission: Apply CONFIDENCE multipliers if applicable
            if client_name == 'CONFIDENCE' and num_weeks in CONFIDENCE_MULTIPLIERS:
                # Use week-based multipliers for CONFIDENCE
                rate_1000 = CONFIDENCE_MULTIPLIERS[num_weeks]['1000']
                rate_other = CONFIDENCE_MULTIPLIERS[num_weeks]['other']
            else:
                # Use standard rates for all other clients
                rate_1000 = rates['1000']
                rate_other = rates['1600/1400/1200']
    ...
    print(f"📋 Phase 2 Features Added:")
    print(f"   ✓ Plan Counting ({num_weeks} weeks)")
    print(f"   ✓ Harry's Downline Commissions")
